    ...
    def drawCard(self):
        ...
        while drawing:
            ...
            if len(ALLCARDS[colour]) - 1 >= 0:
                number = rnd.randint(0, len(ALLCARDS[colour])-1) # number/element in list of cards
                ...
                DISCARDED[colour].append(ALLCARDS[colour][number]) # Adds drawn card to discarded lsit
...
class Uno:
    ...
    def __validateCardChoice(self, card):
        try:
            if self.currentCard.getColour == self.playerDecks[self.playerQueue.getCurrentPlayer - 1][card - 1].getColour or str(self.currentCard.getNumber) == str(self.playerDecks[self.playerQueue.getCurrentPlayer - 1][card - 1].getNumber) or str(self.playerDecks[self.playerQueue.getCurrentPlayer - 1][card - 1].getColour) == '4':
                return True
        except:
            return False
    ...
    def __drawNewCard(self, n, punish):
        ...
        for i in range(n):
            ...
            if not punish:
                print(f'\033[1;37;40mCard drawn: ')
    ...
    def __enactCardAction(self, card):
        ...
        cardEnacted = self.playerDecks[self.playerQueue.getCurrentPlayer - 1][card - 1]
        if cardEnacted.getNumber in actions:
            self.punish = cardEnacted.getNumber
            if cardEnacted.getColour == 4:
                picking = True
                while picking:
                    answer = input('What colour do you want to change the current card to? (r, y, b, g)').lower()
    ...
    def __displayCard(self, card):
        ...
        if card.getColour < 4:
            ...
            col = 37
        num = card.getNumber
        print(f'\033[1;{col};40m ' + f'\033[1;{col};40m_'*(2+len(str(num))))
        print(f'\033[1;{col};40m|'+f'\033[1;{col};40m '*(2+len(str(num)))+'|')
        print(f'\033[1;{col};40m| '+ str(num) + f'\033[1;{col};40m |')
        print(f'\033[1;{col};40m|'+f'\033[1;{col};40m_'*(2+len(str(num)))+f'\033[1;{col};40m|')
    ...
    def __playTurn(self):
        ...
        print('\033[1;37;40m='*(14+len(str(self.playerQueue.getCurrentPlayer))))
        print(f"\033[1;37;40mPlayer {self.playerQueue.getCurrentPlayer}'s turn")
        print('\033[1;37;40m='*(14+len(str(self.playerQueue.getCurrentPlayer))))
        print(f"\033[1;37;40mCurrent card: ")
        self.__displayCard(self.currentCard)
        print(f"\033[1;37;40mPlayer {self.playerQueue.getCurrentPlayer}'s deck: ")
        self.__displayDeck
        while picking:
            print("\033[1;37;40mSelect card to play (i.e., 1, 2, 3, ...) or draw new card (any other input)")
            answer = input()
            try:
            ...
            if self.__validateCardChoice(cardToPlay):
                self.currentCard = self.playerDecks[self.playerQueue.getCurrentPlayer - 1][cardToPlay - 1]
                self.__enactCardAction(cardToPlay)
                self.playerDecks[self.playerQueue.getCurrentPlayer - 1].pop(cardToPlay - 1)
                picking = False
            else:
                print('\033[1;37;40mYou picked a card that does not exist, want to pick again? If not a new card will be drawn. (y/n),')
                ans = input()
                if ans.lower() == 'n':
                    picking = False
    ...
    def startGame(self):
        ...
        while playing:
            ...
            if self.__checkWin():
                ...
                self.playerQueue.nextPlayer()
        print(f'\033[1;37;40mPlayer {self.playerQueue.getCurrentPlayer} has won!!!')
